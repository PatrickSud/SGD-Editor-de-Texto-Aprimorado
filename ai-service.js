...
async function searchQuickMessages(apiKey, query, messages) {
  ...
  const systemInstruction = {
    parts: [
      {
        text: `Você é um motor de busca semântica para trâmites de suporte. Você receberá uma consulta do usuário e uma lista de trâmites salvos (ID, Título, Conteúdo). Sua tarefa é analisar a intenção da consulta e encontrar os trâmites mais relevantes semanticamente.

            Instruções:
            1. Analise a relevância com base no Título e no Conteúdo.
            2. Retorne APENAS uma lista JSON contendo os IDs dos trâmites mais relevantes, ordenados por relevância (mais relevante primeiro).
            3. A resposta deve ser um JSON válido (array de strings), sem explicações ou formatação extra.
  ...
  try {
    ...
    if (Array.isArray(resultArray)) {
      // Filtra para garantir que os IDs retornados realmente existem (segurança contra alucinações da IA)
